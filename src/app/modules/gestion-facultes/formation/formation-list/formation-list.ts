...
export class FormationList implements OnInit {
    ...
    this.departementService.getAll().subscribe({
      next:deps => {
        ...
        this.service.getAll().subscribe({
          next: data => {
            this.rows = data.map(f => ({
              ...f,
              departementNom: this.departements.find(d => d.id === f.departement_id)?.nom || 'N/A'
            }));
  ...
  edit(row: Formation) {
    this.router.navigate(['/facultes/formation', row.id]);
  }

  delete(row: Formation) {
    Swal.fire({
  ...
  exportPDF() {
    ...
    autoTable(doc, {
      columns: [
        ...
        { header: 'Conditions d\'acces', dataKey: 'conditions' },
        { header: 'Durée (années)', dataKey: 'duree' },
